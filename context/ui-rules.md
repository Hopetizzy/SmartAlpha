...
Concise rules for building the Smart Alpha UI. These rules cover the most important patterns and constraints to keep the UI consistent without over-specifying every detail.
...
Nav items: Dashboard, Settings.

- Active item: `color: #101828`, font-weight 600, 14px
- Inactive item: `color: #6A7282`, font-weight 500, 14px
...
Every content section (tables, feeds, settings panels) lives in a card.
...
border-radius: 12px
padding: 24px
box-shadow: 0px 1px 3px rgba(0,0,0,0.05), 0px 1px 2px -1px rgba(0,0,0,0.05)
```
...
Profit/Loss trend badges use `border-radius: 4px` (not pill) with success/error backgrounds.
...
## Tables (Live Alert Feed / Leaderboard)
...
Numeric indicator accompanied by a color:

- 0-30: `#10B981` (green / low risk)
- 31-70: `#FF8904` (orange / medium risk)
- 71-100: `#EF4444` (red / high risk)
...
- CTA button if there's a logical next action (e.g., "Add Wallet to Watchlist")
